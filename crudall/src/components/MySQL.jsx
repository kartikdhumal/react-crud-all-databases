...
function MySQL() {
  ...
  const deleteWork = async (workId) => {
      try{
       await axios.delete(`http://localhost:2500/deleteworkssql/${workId}`);
       getData();
      }
      catch(error){
        console.error(error);
  ...
  const getData = async () => {
    const response = await axios.get('http://localhost:2500/workssql');
  ...
  const addwork = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:2500/addworksql', {work : work});
  ...
  return (
    <div className='w-full h-full flex flex-col justify-center pt-28 items-center'>
      ...
      <label className='py-3'> CRUD - MySQL </label>
          ...
          works ? <>
           {
            works.map((work,index) => (
              <tr key={work.id}>
                ...
                <td className='text-center'><NavLink to={`/updateworksql/${work.id}`} className='text-blue-500'>Edit</NavLink></td>
