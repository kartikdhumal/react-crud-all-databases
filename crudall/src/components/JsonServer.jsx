...
function JsonServer() {
  ...
  const addwork = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:3001/works', {work : work});
        alert('Data Added');
        setWork('');
        getData();
    }
  ...
  return (
    <div className='w-full h-full flex flex-col justify-center pt-28 items-center'>
      ...
      <label className='py-3'> CRUD - JSON Server </label>
          ...
          works ? <>
           {
            works.map((work,index) => (
              <tr key={work.id}>
                ...
                <td className='text-center'><NavLink to={`/updatejsonwork/${work.id}`} className='text-blue-500'>Edit</NavLink></td>
